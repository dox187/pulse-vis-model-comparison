...
fn main() {
    ...
    let term_width = (tui.width() as usize).saturating_sub(2).max(20);
    let term_height = (tui.height() as usize).saturating_sub(3).max(8);
    let buffer_size = term_height.max(8).max(256);
